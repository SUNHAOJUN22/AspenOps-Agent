...
""",
    ...
    text = replace_once(text, '            elif calibration.status != "accepted":', '            elif resolved_calibration.status != "accepted":')
    text = replace_once(
        text,
...
""",
    ...
    text = replace_once(text, '    validation_ceiling = "STRUCTURE_ONLY"', '    validation_ceiling: Maturity = "STRUCTURE_ONLY"')
    text = replace_once(
        text,
...
def main() -> int:
    ...
    EDGE_OUTPUT.write_text("".join(path.read_text(encoding="utf-8") for path in edge_parts), encoding="utf-8")
    return 0
