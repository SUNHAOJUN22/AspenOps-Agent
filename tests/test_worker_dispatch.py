...
def test_worker_dispatch_operations(tmp_path: Path) -> None:
    ...
    opened = _dispatch("open", {"path": str(case)}, backend, accessor)
    ...
    assert len(_dispatch("set_many", {"writes": writes}, backend, accessor)) == 1
    _dispatch("reinitialize", {}, backend, accessor)
    run = _dispatch("run", {}, backend, accessor)
    ...
    assert _dispatch("get_many", {"reads": reads}, backend, accessor)[0]["value"] > 0
    ...
    assert _dispatch("diagnose", {}, backend, accessor)["path_cache_size"] >= 1
    saved = tmp_path / "saved.json"
    _dispatch("save", {"path": str(saved)}, backend, accessor)
    assert saved.exists()
    assert _dispatch("close", {}, backend, accessor) is None

    with pytest.raises(WorkerError):
        _dispatch("unknown", {}, backend, accessor)
