    ...
    def from_dict(cls, data: dict[str, Any]) -> ResearchStudyDocument:
        ...
        if schema != RESEARCH_SCHEMA:
            raise ResearchValidationError(
                f"research document.schema must be {RESEARCH_SCHEMA}"
            )
        return cls(
            schema=schema,
            ...
            datasets=tuple(
                ...
                for index, item in enumerate(
                    _sequence(mapping.get("datasets", []), "datasets")
                )
            ),
            targets=tuple(
                Target.from_dict(_mapping(item, f"targets[{index}]"))
                for index, item in enumerate(
                    _sequence(mapping.get("targets", []), "targets")
                )
            ),
            parameters=tuple(
                Parameter.from_dict(_mapping(item, f"parameters[{index}]"))
                for index, item in enumerate(
                    _sequence(mapping.get("parameters", []), "parameters")
                )
            ),
            assumptions=tuple(
            ...
            claims=tuple(
                ...
                for index, item in enumerate(
                    _sequence(mapping.get("claims", []), "claims")
                )
            ),
        )
